...
Fork of [pstack](https://github.com/cursor/plugins/tree/main/pstack) by Lauren Tan ([poteto](https://x.com/poteto)). Modified for personal use. Continuously changing.

Maintained by Alen Peršić. Upstream pstack is Lauren's answer to AI slop: write less, higher-quality code, and parallelize only when agents apply real engineering principles. This fork keeps that machinery and routes rigor through [`/nax-mode`](./skills/nax-mode/SKILL.md).
...
**cursor gives you the best of all worlds.** every frontier model has its strengths and weaknesses. use any model with pstack. many skills use multi-model workflows on purpose.

## install

this checkout is a local Cursor plugin named `guide-to-the-code`. for the upstream plugin:
...
2. use [`/nax-mode`](./skills/nax-mode/SKILL.md) whenever you're doing anything that requires rigor.

new here? the [guide](./docs/guide/README.md) walks you through a first real task, from setup and prompting through verification and overnight runs.
...
use [`/nax-mode`](./skills/nax-mode/SKILL.md) at the start of a task. it reads your request, picks from a set of playbooks, and runs the other skills as the steps need them.

### just use [`/nax-mode`](./skills/nax-mode/SKILL.md)
...
```
/nax-mode this pr has a subtle bug where the scroll drifts every 750ms even when idle. repro
first, then fix and verify.
```

```
/nax-mode i'm going to bed. land the stack even if ci flakes. i want everything merged by
morning.
```
...
| playbook | for |
|---|---|
| [investigation](./skills/nax-mode/playbooks/investigation.md) | a read-only question. how does x work, why was y built this way, are we sure. |
| [bug fix](./skills/nax-mode/playbooks/bug-fix.md) | reproduce a defect, root-cause it, and fix with runtime evidence. |
| [perf](./skills/nax-mode/playbooks/perf-issue.md) | trace a measured slowness and improve it against a baseline. |
| [hillclimb](./skills/nax-mode/playbooks/hillclimb.md) | sustained, scientific improvement of one metric against a target, looping hypotheses with before/after measurement and one accepted win per iteration. |
| [runtime forensics](./skills/nax-mode/playbooks/runtime-forensics.md) | diagnose a live symptom (leak, idle-cpu spin, glitch) from instrumentation. |
| [trace forensics](./skills/nax-mode/playbooks/trace-forensics.md) | diagnose a captured profiling artifact (cpuprofile, trace, spindump, heap snapshot). |
| [feature](./skills/nax-mode/playbooks/feature.md) | new or changed behavior, built from a named data shape. |
| [refactoring](./skills/nax-mode/playbooks/refactoring.md) | a behavior-preserving change to structure or shape. |
| [prototype](./skills/nax-mode/playbooks/prototype.md) | a throwaway sketch to make a design or behavioral decision cheaply, or to settle an empirical fork by observing it. |
| [visual parity](./skills/nax-mode/playbooks/visual-parity.md) | pixel-exact ui equivalence between two implementations. |
| [design](./skills/nax-mode/playbooks/design.md) | greenfield ui, redesign, screenshot-to-code, or a named aesthetic. not pixel matching, not a throwaway sketch. |
| [authoring a skill](./skills/nax-mode/playbooks/authoring-a-skill.md) | writing or editing a SKILL.md. |
| [eval](./skills/nax-mode/playbooks/eval.md) | test how a skill or prompt change affects agent behavior, blinded. |
| [babysit](./skills/nax-mode/playbooks/babysit.md) | drive a pr or a stack to merge-ready: conflicts, review threads, ci. |
| [shipping](./skills/nax-mode/playbooks/shipping.md) | independently verify a green stack, then land the contiguous verified run through github (`gh`) or origin. |
| [autonomous run](./skills/nax-mode/playbooks/autonomous-run.md) | drive a long task to completion without stopping. |
| [orchestrate](./skills/nax-mode/playbooks/orchestrate.md) | a standing project handed to one coordinator chat: multi-day, many stacked prs, fleets of subagents. |
| [autopilot-full](./skills/nax-mode/playbooks/autopilot-full.md) | run independent prs to merged with one owner per pr and root verification of each merge-ready head. |
| [autopilot-stack](./skills/nax-mode/playbooks/autopilot-stack.md) | build and verify one linear base-branch stack for the operator to review and land. |
| [session pickup](./skills/nax-mode/playbooks/session-pickup.md) | resume or take over a prior agent's in-flight work. |
| [pause safely](./skills/nax-mode/playbooks/pause-safely.md) | suspend in-flight work cleanly so it can be resumed later. |
| [multi-phase plan](./skills/nax-mode/playbooks/multi-phase-plan.md) | work that spans phases or stacked PRs. |
| [worktree cleanup](./skills/nax-mode/playbooks/worktree-cleanup.md) | reclaim disk by pruning merged or abandoned worktrees and stale ios simulators, safety-gated. |
...
2. matches your task to a [playbook](./skills/nax-mode/playbooks/) and copies the steps in verbatim.
3. routes to the other skills as the steps fire.
4. writes unslopped replies framed for the consumer and the maintainer.

the full rules and playbooks live in [`skills/nax-mode/SKILL.md`](./skills/nax-mode/SKILL.md).

[`/nax-mode`](./skills/nax-mode/SKILL.md) is also a sticky mode: once entered it stays on across turns, applying itself when a playbook matches or the task needs rigor and staying out of the way otherwise. opt out any time by saying so.

[`/nax-mode`](./skills/nax-mode/SKILL.md) works extremely well with cursor's `/loop` command. you can make cursor work for many hours without sacrificing rigor.

## skills

[`/nax-mode`](./skills/nax-mode/SKILL.md) runs most of these for you when a step needs them (`how`, `why`, `architect`, `arena`, `swarm`, `interrogate`, `deslop`, `unslop`, `no-comments`, `technical-writing`, `tdd`, and the principles). the table below is for when you want one directly:
...
| skill | use it when |
|---|---|
| [`/nax-mode`](./skills/nax-mode/SKILL.md) | default entry point for any non-trivial task. |
...
| [`/maintain-verification-skill`](./skills/maintain-verification-skill/SKILL.md) | your verify skill's feature map has drifted from the app. source wave + one live pass, then one PR of proven corrections. |
| [`/design-taste-frontend`](./skills/design-taste-frontend/SKILL.md) | landing pages, portfolios, and redesigns that should not look templated. loaded by the Design playbook. |
| [`/image-to-code`](./skills/image-to-code/SKILL.md) | generate or analyze site references, then implement the frontend to match. |
| [`/web-design-guidelines`](./skills/web-design-guidelines/SKILL.md) | audit UI against Vercel Web Interface Guidelines. |
| [`/design-style-catalog`](./skills/design-style-catalog/SKILL.md) | fetch one named aesthetic from Awesome Design Skills by slug. |
...
mostly i type [`/nax-mode`](./skills/nax-mode/SKILL.md) at the start of a task and let it route to a playbook. the other skills fire as the steps need them. a few i reach for directly.
...
```
bug fix:           /nax-mode this pr has a subtle bug where the scroll drifts every 750ms even
                   when idle. repro first, then fix and verify.
perf:              /nax-mode a big list takes a second or two to load even though we virtualize.
                   run a cpu trace and tell me why.
feature:           /nax-mode build a small feature behind a feature flag. verify it really works.
prototype:         /nax-mode build two prototypes of the markdown renderer so we can compare.
                   spawn an agent for each.
multi-phase:       /nax-mode open source these skills as a plugin. nothing internal leaks, work
                   in a temp dir, show me the dependency graph first.
overnight run:     /nax-mode i'm going to bed. land the stack even if ci flakes. i want
                   everything merged by morning.
babysit:           /nax-mode check on pr 123. anything outstanding?
visual parity:     /nax-mode the row spacing is too tall when this flag is on. the second image
                   is correct. repro and fix until it matches.
design:            /nax-mode design a landing page for this product. not generic AI slop.
                   screenshot the result.
figure it out:     /nax-mode i'm stepping away. migrate every caller from the synchronous store
...
## the `nax-agent` and Comment Sicko subagents

pstack also ships a subagent that runs my style end to end. spawn it from a parent agent via [`subagent_type: "nax-agent"`](./agents/nax-agent.md). it reads `nax-mode` in full, including its inline principles index, before doing any work. substituting `generalPurpose` skips that read and drifts.

[`/nax-mode`](./skills/nax-mode/SKILL.md) and [`subagent_type: "nax-agent"`](./agents/nax-agent.md) route through the same wrapper.
...
twenty-three short skills, one principle each. `nax-mode` indexes them inline and reads that index at task start. the standalone files are there so other skills can reference a principle by name, and so the index can point at the full rule for each.
...
| [attack-the-premise](./skills/principle-attack-the-premise/SKILL.md) | core | When two or more fixes that share one premise have failed the same gate, question the premise after a census of which actors hold the imbalance. |
...
a few things `nax-mode` references but doesn't bundle:

- `control-cli` (for CLIs and TUIs) and `control-ui` (for browser, Electron, web) ship in the `cursor-team-kit` plugin.
- `/create-skill` is a cursor built-in. cursor also ships a built-in `/babysit`; inside `nax-mode`, the [babysit playbook](./skills/nax-mode/playbooks/babysit.md) supersedes it for pr-status requests.

[`/deslop`](./skills/deslop/SKILL.md) ships in this plugin. install `cursor-team-kit` alongside if you want the control skills.

## why are there no planning skills?

cursor already has a great plan mode which works great with pstack. but personally, i don't believe in planning. the best spec is code. if you do want to make a plan, [`/nax-mode`](./skills/nax-mode/SKILL.md) covers it, but it's not a default. 

## make it yours

`nax-mode` is this fork's default routing skill. you may not want exactly that.

type [`/automate-me`](./skills/automate-me/SKILL.md). it mines your recent transcripts, drafts a `<your-name>-mode` skill from how you've actually worked, and routes through pstack underneath. you keep pstack as the base and end up with your own routing skill alongside `nax-mode`.
...
MIT. Third-party design skills keep their own licenses. See [CREDITS.md](./CREDITS.md).
