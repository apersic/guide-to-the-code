...
Write `~/.cursor/rules/pstack-models.mdc` with `alwaysApply: true` and one line per role, using the same labels nax-mode uses. Overwrite the whole file so re-runs stay idempotent. Shape:
