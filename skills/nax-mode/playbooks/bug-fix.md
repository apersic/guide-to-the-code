...
5. Order the work so the failing repro is visible before the fix. See the **tdd** skill for the failing-test-first cadence when the bug has a cheap local test path. Skip it when the test would be expensive, integration-heavy, or unclear.
   This is the canonical **sequence-verifiable-units** principle skill, the failing test first and the fix on top. Do not commit or open a PR unless the user asked.
6. Present the work. If the user asked to commit or open a PR, run **Opening a PR**. Otherwise stop here.
