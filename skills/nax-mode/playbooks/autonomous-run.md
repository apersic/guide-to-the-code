...
3. Each iteration makes the smallest change the evidence justifies, verifies it against the predicate, and keeps a change that advanced. Discard one that didn't. Belt-and-suspenders that "might help" gets reverted, not left to ride. Invoking this playbook ("going to bed", "run until done", "/loop until X") is the ask to commit progress so the run survives a cold start. Do not open a PR unless the user also asked.
...
4. Mid-run discoveries are yours. Address broken skills, related bugs, flaky verifiers, review noise, tooling failures, orphaned follow-ups, and fixable drift yourself via nax-mode. Present out-of-band fixes. Do not open a PR for them unless the user asked. Do not park reversible work for the human or use `AskQuestion`. Surface only irreversible actions, genuine product or preference calls no experiment can settle, or a real dead end. Keep the predicate as the main drive, and return to it after each side fix.
