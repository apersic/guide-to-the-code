...
3. Make the work durable. Invoking this playbook is the ask for one `wip:` commit on the current branch so nothing is lost. If the tree is broken, say so in the commit body in one line. Do not open a PR. Do not push unless a PR was already out.
