---
name: Nax Mode
description: nax's agent style for concise, detailed responses, deliberate subagents, unslopped prose, simple code, and verified work. Use for nax, /nax-mode, or requests to work in this style.
disable-model-invocation: true
mode: true
icon: crown
color: yellow
reminder: New task? Playbook match or rigor needed -> apply /nax-mode. Casual turn or user opts out -> don't.
---
...
- Broken skill mid-task → fix it, keep it out of the current feature diff, and present it. Do not commit or open a PR unless the user asked. Don't block. Don't silently work around it.
- Long, autonomous, or multi-phase work, or any task the user steps away from to review later ("going to bed", "trust it when i'm back", "/loop until X") → a decision trail via the **show-me-your-work** skill. Keep it local unless the user asked to commit.
...
**Use `subagent_type: "nax-agent"` for any subagent you spawn inside a playbook step** (code-writing delegates, ad-hoc helpers). `/nax-mode` and `nax-agent` route through the same wrapper. Routed workflow skills (`how`, `why`, `interrogate`, `reflect`, `swarm`) set their own `subagent_type` for diverse-model review; respect what the skill prescribes, don't override to `nax-agent`.
...
Every playbook ends with a reply written this way. Include a PR link as `https://github.com/<owner>/<repo>/pull/<number>` only when a PR exists. The per-playbook lines below name only the content unique to that playbook.
...
- **Hillclimb.** Sustained, scientific improvement of one metric against a target: loop hypotheses with before/after measurement, a decision log, and one accepted win per iteration. Distinct from Perf issue, which is a one-off fix. `playbooks/hillclimb.md`.
...
- **Design.** Greenfield UI, redesign, screenshot-to-code, or a named aesthetic. Dispatch is surface (marketing vs product UI) by kind. Not pixel-exact matching (Visual parity) and not a throwaway layout sketch (Prototype). `playbooks/design.md`.
...
- **Opening a PR.** Only when the user asks to open a PR, commit, or stack the work. Other playbooks present the diff and stop. `playbooks/opening-a-pr.md`.
